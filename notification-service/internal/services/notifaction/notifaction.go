...
func (n *Notification) SenEmail(ctx context.Context, to, senderName, content string) error {
	const op = "Service.SenEmail"
	log := n.logger.With(slog.String("Method", op))
	log.Info("SenEmail")
	defer log.Info("SenEmail Completed")
	email1 := entity.EmailNotificationReq{
